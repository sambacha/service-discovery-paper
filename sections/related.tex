...
There exists several network discovery protocols for local area networks,  such as  Simple Service Discovery Protocol (SSDP)~\hl{missing ref},  basis of the discovery protocol of Universal Plug and Play (UPnP)~\hl{missing ref},  or DNS-based service discovery,  to access to services that other devices provide,  such as printers,  webcams,  HTTPS servers, and other mobile devices.  
However for service discovery in Internet-wide environments,
is usually used centralized setups to provide good latency and performance.
UDDI~\hl{missing ref} has been
recognized as the most popular discovery mechanism for
Web services
