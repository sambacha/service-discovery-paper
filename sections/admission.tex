...
\mk{Shall we drop the ID part for simplicity? We can uniquely focus on topics/IPs. It should not influence the results.}

\mk{Shall we rename the table to: searcher, advertisers and registrar tables? Currently "register table" might be confusing. The current topic table, maybe should be also renamed to something else than table? It's pretty different from the other tables that are routing tables, rather than an array/matrix.}
...
\para{Topic Table}
Registrars store received ads locally in a data structure called a \emph{topic table}. Each ad stored in the topic table has an associated expiry time, after which the ad is automatically removed. Once an ad is added to the table, the expiry time is set to a fixed value $a$\mk{TODO:make sure we're consistent with ad lifetime, expire time etc. + use the same symbols}. The topic table acts as a \emph{first in, first out} queue for ads. The total size of the topic table is limited by $C_\textit{topic\_table}$. \sysname does not impose topic/IP/ID-specific limits on the content of the \emph{topic table}. \mk{Explain why?}
