...
The advertiser \er{the registrar?} calculates a new waiting time, based on the current content of
...
\er{I am not sure to understand anymore why we need to define $w$? Is it different than $t_\textit{waiting}$?}
...
\begin{algorithm}[]%
    ...
    \begin{algorithmic}[1]%
        ...
        \item[]
        \Procedure{Advertise}{topic}
            \State $\algvar{regTable.initFromRoutingTables()}$
            \For{$\algvar{bucket}$ in $\algvar{regTable}$}
                \If{$\algvar{regTable[bucket].size} < K_{register}$}
        ...
        \EndProcedure
        \item[]
        \Procedure{AdvertiseSingle}{registrar, ad}
            \While{True}
        ...
        \Procedure{Lookup}{topic}
            ...
            \For{$\algvar{bucket}$ in $\algvar{lookupTable}$}
                \For{$\algvar{i}$ in $\algvar{range(0, K_{lookup})}$}
                    ...
                    \If{$\algvar{foundPeers.size} \ge N_{lookup}$}
...
%lookup
%while not enough peers found
% for every bucket in the lookup table
% for i in 1 to K_lookup
% send a lookup request to a random node in the bucket
% add to peers_found