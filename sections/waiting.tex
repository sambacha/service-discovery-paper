...
\ramin{There is no motivation given for $b\cdot a$. Maybe remove it from the occupancy score (and add it as a factor to the equation for $w(IP,ID,topic)$?}  
...
\ramin{The above as an equation :-) $$score(IP)=\sum_{i=0}^{32} p_{\geq i}^i$$ where $p_{\geq i}$ is the number of IP addresses in the table sharing a prefix with $IP$ with a length of at least $i$}
...
\ramin{Following the above explanation, shouldn't the divisor be $\frac{rootCounter^{33}-1}{rootCounter-1}$?}
\michal{Hmm, I don't think so. If you consider a tree with only 2 levels (root + children) and rootCounter = 1, the max score is 2 ($1^0 + 1^1$). Following your formula, it'd be $\frac{0}{0}$, right? Happy to discuss if I'm not getting something here.}
\ramin{Isn't the similary score very small most of the time? Even if the new IP shares 31 bits with all the other IP addresses in the table, the similary score would be only around $\textit{rootCounter}^{-P_\textit{IP}}$}
\michal{indeed it almost never goes to $1$. However, using high values of $P_\textit{IP}$ helps to solve that. The formula seems to works pretty well with correct parameters during simulations but I'm happy to discuss improvements here. }
