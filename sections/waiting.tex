...
As the ad cache fills up, the score will be amplified \er{right term?}\mk{I think so. The score increases as the ad cache fills up} by the divisor of the equation. 
...
As a result, the root counter stores the number of all the \er{different instead of ``all the''?}\mk{should be "all the". If the same IP address is added twice, the counter increases by 2.} IP addresses in the ad cache, its $0$ successor stores the number of the IP addresses starting with $0$, root's $1$ successor stores the number of the IP addresses starting with $1$ and so on. 
