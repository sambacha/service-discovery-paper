...
An \emph{advertise table} is similar to the routing table used by the Kademlia protocol, as described in \Cref{sec:background}.
...
Each bucket aims to contain \emph{at least} $K_\textit{register}$ potential registrars for every distance (bucket) from the topic ID (\Cref{fig:advertise_table}).
...
Theses buckets are initially filled from the local routing table. % organized by the distance from the topic hash. %, and later filled with new nodes discovered during the registration process.
...
\subsection{Service discovery operation}
\label{sec:lookup}

The service discovery process in \sysname shares similarities to the ad registration procedure.
