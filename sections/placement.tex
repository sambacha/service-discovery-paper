...
\michal{It's basically FIND\_NODE but based on a different table (registration table, rather than the routing table) and performing the registration on top of traversing the DHT. IMO it's easier to have a separate name here.}
...
\sergi{Criticial: How do you choose the registrars}
\sergi{how do you fill up search table. 
it is initialized from routing table but also nodes are added from registrations messages and find messages}
\sergi{do we send find messages when empty buckets?}
\sergi{how do you select registrars in each bucket}
\sergi{Do we query all the registrars in a bucket or just a subset?}
\sergi{How many queries per node/ how many parallel queries}
\sergi{Where do we stop?}
\sergi{Results filtering to produce results from more than X regsitrars}
\sergi{Aggregate results from multiple registrars from multiple buckets?}

The furthest buckets are also the largest. For instance, bucket 0 contains 50\% of all the honest registrars. An attacker placing malicious registrars in those buckets would require significant resources to eclipse the procedure. Searchers progressively move towards smaller buckets near the topic hash guaranteeing successful discovery even for unpopular topics.
