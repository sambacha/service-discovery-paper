...
Each lookup requires a topic-specific \emph{search table} described in Section\Cref{sec:struct}
...
Each queried registrar responds with a list of $N_\textit{return}$ topic-specific advertisers the registrar know of. If the total number of topic-specific registrations is larger then $N_\textit{return}$, the registrar return a random subset. 
...
We investigate the impact of both parameters in \Cref{sec:eval}.
\sergi{Not sure there will be space in the evaluation for that,  in case not we can point to the github report}.
...
\sergi{I think $N_\textit{return}$ shouldnt be configurable since it is limited by the maximum number of nodes returned in a message which i think is 15 or 16.  I think $N_\textit{lookup}$ can be configurable, but never smaller that a value multiple of $N_\textit{return} max$}
