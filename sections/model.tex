...
A DHT node can participate in advertising and searching for multiple topics and they will use the same ID and IP for all topics.
...
\michal{The section below is weird. We go into a lot of details about connections (that we didn't introduce) and some blockchain-specific attacks (which are application-specific). Maybe we should just say that we assume presence of byzantine nodes?}
\dk{I agree with that comment; still I think the specific attacks are helpful for motivating the need to protect against eclipse attacks.}
...
\ramin{Depending on the target conference, it could be useful to put security aspects in a separate, more visible section (e.g. "Threat model" etc.)} 
\dk{Yes. A separate section "threat model" introducing the discussed concepts (see Michal's comment) would also make this easier to understand. Since protecting against / mitigating eclipse attacks is an important contribution of this paper, imo, a threat model section is important.}

%\onur{maybe we should indicate somewhere that DHT and discovery are separate layers and are largely independent except initially DHT is used for bootstrapping the discovery.}
