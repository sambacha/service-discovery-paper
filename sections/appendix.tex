...
d_b & = & \frac{\frac{N_b\cdot K_{register}}{N/2}}{1 + (b + \frac{d_b}{d}) \cdot (1 - \frac{d}{n})^{-P_{occ}}}\\
\label{eq:da_fair2}
d_X & = & \frac{N_x}{1 + (b + \frac{d_x}{d}) \cdot (1 - \frac{d}{n})^{-P_{occ}}}
\end{eqnarray}
...
The probability to reach the last registrar is then $p_{1..u}(Resp< N_{lookup})$, where $u$ is the number of buckets.

% TODO:  explain calculation of overlaps

\subsection{Security}

The probability that a searcher is eclipsed when looking up a topic in $t$ buckets is the probability to only receive malicious ads from the contacted registrars. If the network contains $n_a$ malicious nodes, the probability that a searcher will contact a malicious registrar in a bucket $i$ is $\frac{n_a}{N}$, assuming a uniform distribution of the malicious nodes.
