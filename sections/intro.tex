...
\begin{enumerate}
    \item We consulted it with Felix, at agreed that we can call our think Discv5, while calling the previous one Discv4. We should just explain that both Discv5 and Discv4 have other components (e.g., slightly different encrypted communication) that we don't consider here.
    \item In progress
...
\mk{In the analysis we calculate what's the probability of an operation returning only malicious results}
