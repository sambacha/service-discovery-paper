...
\sr{why there is an increase for \discv with the number of nodes in the simulation and not the number of topics??? shouldn't be the opposite?}
...
We evaluated the eclipse attacks targeting two specific topis, the most popular topic and the least popular topic.
The most popular topics has 3978 nodes registering for this topic and the least popular has only 19 nodes.
We evaluated the attacks for the most popular and least popular 

\sr{TODO: show protocols in the same order for all figures}
...
In Figure~\ref{fig:eclipse_distribution_t0} we show the malicious nodes discovered for all lookups for all protocols using different distribution of the identifiers of the malicious nodes,  when attacking the most popular topic (with 3978 nodes).  In the first option the malicious nodes ids are artificially generated with a small distance to the topic hash id.  In the second option malicious nodes identifiers are uniformly distributed.  In the figure we can observe that for \altname and \altnameticket the percentage of eclipses are superior to 90\% because most of the nodes close to topics identifiers are nodes controlled by the attacker.  
...
When attackers are uniformly distributed in the network, eclipses are reduced to close to 50\% of the lookups.
...
When the least popular topic is attacked (only 19 nodes),  Figure~\ref{fig:eclipse_distribution_t199},  for  \altname and \altnameticket the eclipses are even higher, reaching a 100\% of eclipses when placing malicious nodes close to the topic id, and reaching a 94.7\% when distributing malicious nodes uniformly.  
Remember the number of attackers is 1000,  against the 19 valid nodes.
For \discv, the number of malicious nodes returned is very low for the most popular topic when attackers are placed close to the topic hash.  When Sybil nodes are uniformly distributed the number of malicious nodes returned increases,  however the eclipses only reach a 0.1\%.
...
When attacking the least popular topic, \discv is the more resistant for non uniform malicious nodes identifiers with only 21.1\% of eclipses, but when using  malicious nodes identifiers  uniformly distributed the resistance using \discv is worse than \sysname with  78.9\%.
The resistance of \discv to Sybil attacks is obtained with the important trade-off of very low efficiency when finding nodes for specific topics, specially for low popular topics.  It is very likely that, when doing lookups using \discv for very low popular topics, no nodes are found or just a few of them.  However, this means when hitting a malicious node during lookup,  even if it is difficult to find it, it is likely with a single query to a malicious node, the node querying will be eclipsed.
For \sysname, the number of eclipses are very low for the most popular topic. 
The eclipses are 0\% when Sybil nodes are placed close to topic id and 0.5\% when are uniformly distributed. 
However when attacking the least popular topic the eclipses increase to 52.6\% and 59.9\% respectively.
This is due to the very low number of valid nodes (19),  that will be more difficult to find than the attackers (1000 nodes, reusing 100 IP addresses).  
But event he high number of attackers in almost half the cases valid nodes are also found along the evil nodes.
...
\caption{Y-axis: Malicious nodes discovered and percentage eclipsed nodes using uniform distributed sybil identities vs generating node ids close to topic id,   when attacking the least popular topic (19 nodes).}
\label{fig:eclipse_distribution_t299}
\end{figure}

In Figure~\ref{fig:eclipse_evil_t0} we show the malicious nodes discovered for all lookups for all protocols using different  number of Sybil nodes,  when attacking the most popular topic,  and in Figure~\ref{fig:eclipse_evil_t299} we show the malicious nodes discovered for the least popular topic.  In both figures attackers identifiers are generated uniformly and attackers use a pool of 100 IP addresses. 
In Figure~\ref{fig:eclipse_evil_t0} we observe \discv is the protocol with the lowest number of eclipses and with the best distribution of malicious nodes returned during lookups.  However \sysname performs very close to \discv, having only 2\% of eclipses when there are 2500 attackers in the network.
\altname and \altnameticket has the worst performance,  having 79.7\% and 81.4\% of eclipses when when there are 2500 attackers.
In Figure~\ref{fig:eclipse_evil_t299}, when attacking least popular topic, we observe \sysname is the more resistant to eclipsing.
This is due to it is the protocol that is able to discover more nodes with a higher diversity and is the protocol that will find easier the any of the few valid nodes in the network, and therefore it will be not eclipsed.
...
\caption{Y-axis: Malicious nodes discovered and percentage eclipsed nodes for different number of sybil nodes used in the attack,  when attacking the least popular topic (19 nodes).}
...
\caption{Y-axis: Malicious nodes discovered and percentage eclipsed nodes for different number IP addresses used in the attack,  when attacking the least popular topic (19 nodes).}
