...
\begin{enumerate}
\item \discv is the current discovery service~\cite{discv4} of Ethereum. In \discv, the DHT is merely used to perform \emph{random walks} in the P2P network. \er{not consistent with the presentation of \discv as a collection of protocols before, see comments in intro.} \oa{now we introduce discv4 as merely a discovery protocol, so we should be good now.}
...
In this approach, peers store their ads under desired topics at the k closest \er{consistency: in background, $k$ is used for the closest $k$ peers to a topic; and $N$ is defined in model as the number of nodes in the system.} \oa{good catch - I changed N to k} (by DHT distance metric) nodes to the corresponding topic hashes. In \altname, registrars accept all incoming ad registrations right away (\ie with no waiting time) and use the Last Recently Used (LRU) replacement policy.
...
As similarly done in the IPFS system, we use k=20 peers (closest to the topic hashes) for storage of ads in both \altname and \altnameticket approaches.

% On the other hand, t
The topic lookup process in \altname and \altnameticket involve first finding k closest peers to the topic using the key lookup process of the Ethereum DHT as discussed in \Cref{sec:background:dht}. Once its lookup operation terminates with a stable list of k closest peers to the desired topic, a searcher locally sorts those peers based on their distance (using Kademlia's XOR metric) to the topic's hash. Finally, the searcher queries $\alpha=3$ peers at a time, proceeding with the farthest to the closest of the k peers.
...
\para{Metrics} The simulator reports the following metrics: 
 \begin{itemize}
...
Based on formal analysis (see \Cref{sec:analysis}), we set other \sysname parameters such as the number of registrations placed per bucket $K_\textit{register}= 3$, the occupancy exponent $P_\textit{occ} = 10$, the number of peers to contact per bucket during a topic lookup $K_\textit{lookup}=3$, the maximum number of ads a single registrar returns in a lookup $N_\textit{return}=10$. On the other hand, the total number of ads (\ie $N_\textit{lookup}$) is an application-specific parameter, which we set to 30 based on similar requirements of existing applications such as Geth.
...
\Cref{fig:eclipse_evil} presents the percentage of malicious nodes discovered during a single lookup as a violin plot,  including the percentage of eclipsed nodes after the lookup,  on top of each plot.
...
In \Cref{fig:eclipse_evil}, we observe  \sysname to be the most resistant to eclipsing \sysname, achieving close to a $0$ eclipse rate, even with a high number of malicious nodes.  
...
In \Cref{fig:eclipse_sybil},  we again illustrate the percentage of malicious nodes in the lookup results and the percentage of eclipsed nodes.
