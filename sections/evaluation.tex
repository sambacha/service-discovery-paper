...
\para{Setup} We built a prototype of \sysname in PeerSim~\cite{p2p09-peersim}---a scalable P2P network simulator. In particular, we first implemented the Ethereum DHT by extending a vanilla Kademlia implementation~\cite{peersim_kademlia} with the required changes discussed in ~\Cref{sec:background:dht}. We then built \sysname on top of the Ethereum DHT implementation.
...
In \discv, a node issues three lookups to random destinations using  Kademlia lookup,  and it stores encountered nodes during the random walk in a buffer. The nodes in the buffer are consumed by the upper-layer Ethereum protocols (\eg RLPx~\cite{rlpx} to make new connections, and when the buffer is empty new lookups are initiated to other three random nodes.
...
In the simulations, we verify the impact of the following parameters:
 \begin{itemize}
     \item \textbf{Network Size} - the total number of nodes that participate in the discovery. We set the default value to 25000, as similar number of live nodes were reported by the official Ethereum crawler~\cite{discv4-dns-lists} (\Cref{fig:ecosystem}). We assign each node an IP address and an ID from the crawled ENS records. We observe an average network churn of 3\% across daily crawls equal and therefore we use this value in our simulations to simulate nodes joining and leaving. 
     ...
     \item \textbf{Malicious nodes} - the number of attackers targeting the peers in the discovery system. We experiment with values ranging from 250 and 2500 nodes,  using 1000 for the default value. \oa{These values need to be checked} Each malicious node receives a distinct ID (\ie hash of its public key) generated following either uniformly at random or non-uniformly, \ie pick identifiers that are closer to the hash of the target topic than any honest node. The latter can be achieved in practice through ``brute-force'' generation of public keys until one with a suitable hash is found.
     \item \textbf{Malicious IPs} - the number of IP addresses available to all the attackers. We set its value between 10 and 1000, using 100 for the default value. \oa{Can we justify the choice of 100 IPs - maybe mention the cost of allocating 100 IPs in AWS} In our experiments, we use hypothetical attacker IP addresses that have minimum similarity with the set of real IP addresses used by IP addresses (obtained from the crawled ENS records) in order to obtain an upper-bound on the impact of attacks. %At the time of writing this paper, 99\% of the addresses in the Ethereum network are IPv4, and therefore we use only IPv4 addresses in the simulations.
...
We set all the DHT-related parameters to the ones currently used by Ethereum DHT, \ie there are 17 buckets and upto 16 peers stored in each bucket. \oa{Mention the replacement policy for a bucket}. 
Based on our analysis (see \Cref{sec:analysis} and also Appendix), we set other \sysname parameters such as the number of registrations placed per bucket $K_{register}= 3$, the occupancy exponent $P_{occupancy} = 10$, the number of peers to contact per bucket during a topic lookup $L_\textit{lookup}=3$, the maximum number of ads a single registrar returns in a lookup $N_\textit{return}=10$. On the other hand, the total number of ads (\ie $N_\textit{lookup}$) is an application-specific parameter, which we set to 30 based on similar requirements of existing application such as Geth.
...
In the rest of this section, we use violin plots (\ie density curves) for a compact illustration of both the distribution and the density of data points for each performance metric. For each violin plot, the vertical length illustrates the exact range of observed values, and the width indicates the count of observations (\ie density) at each data point. 
%Also, in some of the plots, we display the maximum observed value at the top of a violin curve for better illustration. 
%Also, for some of the results, we split the violin into two and illustrate the performance with and without attackers' presence in each side. 
...
In the following figures we show violin plots representing the number of malicious nodes returned per lookup using different number of attackers (by default there are 500 attackers) and using different number of IP addresses used in the attack (default is 10) for the same protocols evaluated in the performance simulations.  On top of each violin plot we specify the percentage of nodes eclipsed after a lookup (\ie percentage of lookups where all nodes returned are malicious nodes).
All attackers identifiers are generated uniformly distributed in the address space.
We evaluated the eclipse attacks targeting two specific topics, the most popular topic and a low popularity topic.
...
In Figure~\ref{fig:eclipse_evil_t0} we show the malicious nodes discovered for all lookups for all protocols,  using different number of Sybil nodes (attackers) in the network,  when attacking the most popular topic,  and in Figure~\ref{fig:eclipse_evil_t299} we show the malicious nodes discovered for the least popular topic.  
%In both figures attackers identifiers are generated uniformly and attackers use a pool of 100 IP addresses. 
In Figure~\ref{fig:eclipse_evil_t0}, we observe \discv is the protocol with the lowest number of eclipses and with the best distribution of malicious nodes returned during lookups.  However \sysname performs very close to \discv, having only 0.6\% of eclipses when there are 1000 attackers in the network.
\altname and \altnameticket has the worst performance,  having 26.7\% and 25.9\% of eclipses when when there are 1000 attackers.
In Figure~\ref{fig:eclipse_evil_t299}, when attacking least popular topic, we observe \sysname is the most resistant to eclipsing in all cases.
This is because it is the protocol that is able to discover more nodes with a higher diversity and is the protocol that will find easier  any of the few valid nodes in the network, and therefore it will be not eclipsed.
...
\sysname is very close to \discv performance with eclipses closes to 0\% for the most popular topic, and \altname and \altnameticket is close to 30\% and  eclipses in all cases.
When attacking the low popularity topic with 100 IPs used,  nodes eclipsed are very similar but showing always a  better performance for \sysname.
When the number of IPs used in the attack are lower,  we can appreciate \sysname  has a much higher the resistance to eclipsing,  caused by its ticket regulation, having only 5.9\% eclipses when used a single IP address and a 41.2\% of eclipses when 10 IP addresses used.  This performance is superior to the rest of the protocols that they do no have percentage of eclises below 70\% in any of the cases.
