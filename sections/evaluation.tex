...
We evaluated the traditional key/value store implementation, that we named \altname, using our admission protocol (\Cref{sec:waitingTime}) to accept incoming registrations, and also without any admission protocol were registrations are replaced using  Last Recently Used policy.
...
\sr{I think we should find a name for DHT solution and using consistently in the evaluation because i think can be confusing when we talk about DHT solution because all of them use a DHT.}
...
In the simulations, we verify the impact of the following parameters:
 \begin{itemize}
  ...
  \mk{Do we have any IPv6 addresses? If not we can justify focusing uniquely on IPv4.}
  \sr{Will the ipscore work with the same performance in case of IPv6 addresses?}
 \end{itemize}
...
In~\Cref{fig:regMsgsPerTopic}~and~\Cref{fig:regMsgsPerSize},  we can observe while most of \sysname nodes receive around 500 registration messages,  with just a few receiving up 5k messages in the worst case,  \altname solutions are more spread between 0 and 1000 received messages, but with some peaks up to 43k messages,  and order of magnitude higher than \sysname.
...
\sr{\Cref{fig:regMsgsPerSize} does not show max values. Fix}
\begin{figure}[!h]
\centering
