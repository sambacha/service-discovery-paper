...
We express the waiting time in ad lifetimes $a$ and keep the topic colors consistent with the one used in \Cref{fig:testbed_discovered_search} and \Cref{fig:testbed_time_discovered}.
...
\er{Figure~\ref{fig:testbed_waiting_time} is not reference in the text}
...
\mk{We can reduce/remove some of the metrics below if we properly define them before.}
...
\er{some of the plots still mention TOPDISC instead of discv5}
