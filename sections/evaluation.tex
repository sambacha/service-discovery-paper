...
For large scale experiments we also built a prototype of \sysname in PeerSim~\cite{p2p09-peersim}---a scalable P2P network simulator. % In particular, we first implemented the Ethereum DHT by extending a vanilla Kademlia implementation~\cite{peersim_kademlia} with the required changes discussed in ~\Cref{sec:background:dht}. We then built \sysname on top of the Ethereum DHT implementation.

We compare \sysname against the following approaches:
\begin{enumerate}
\item \discv is the current discovery service~\cite{discv4} of Ethereum. In \discv, the DHT is merely used to perform \emph{random walks} in the P2P network. \er{not consistent with the presentation of \discv as a collection of protocols before, see comments in intro.}
...
In this approach, peers store their ads under desired topics at the N closest \er{consistency: in background, $k$ is used for the closest $k$ peers to a topic; and $N$ is defined in model as the number of nodes in the system.} (by DHT distance metric) nodes to the corresponding topic hashes. In \altname, registrars accept all incoming ad registrations right away (\ie with no waiting time) and use a simple Last Recently Used (LRU) as a replacement policy.
