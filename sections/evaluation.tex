...
\michal{\Cref{fig:msgsPerTopic} I'm not sure this is relevant without splitting into lookup and registration. For instance, we can make discv4 look very bad or very good by simply increasing the frequency of lookups. }
\sergi{I added them into a single figure because the limit space. Moreover at the end the most important is the overall overhead generated by each solution and compare only lookups or only registrations i think is not completely fair with discv4, because there are not regs in discv4.  We can always make discv4 look very bad increasing lookups, but it is supposed we run same number of lookups for all.  But if you think is better we can show it separately.}
...
\caption{Y-axis: Distribution of discovery related (including registration and lookup) messages received by peers for different network size.}
\label{fig:msgsPerSize}
\end{figure}
\michal{\Cref{fig:msgsPerSize}, same here - should split into registration and lookup}
