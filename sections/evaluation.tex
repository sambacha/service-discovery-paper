...
\sr{TODO: explain topic spam (and eclipsing?) attack} 
...
The violin plots are split in two sides. The left side shows the network performance when there is no attack in the network. The right side shows the network performance under a Denial of Service topic spam attack,  described in the beginning of this section.
In the top of the figure we also show maximum values, comparing with and without being attacked when this values go off the figure.
...
In~\Cref{fig:msgsPerSize}~and~\Cref{fig:msgsPerTopic}, we show the total number of messages (overhead) created in the network by all protocols, that is total number of messages received by node during registration and lookup process. For \discv, only lookup generated messages are shown since there is no registration process in the protocol.
We observe the total number of messages during the simulation but normalised per advertisement period (\ie 15 minutes).
...
DHT-based solutions show the best discovery performance since they go direct towards the nodes (close to the topic id) where its known most of registrations will be, without taking into account network load or security resistance. 
However \sysname is very close to DHT protocols performance
...
\Cref{fig:discoveredPerSize} presents the number of peers discovered during a single lookup operation with increasing size of the network. With a fixed amount of topics, each application-specific network grows and for all the protocols, it is easier to find the required amount of nodes.
 However, \discv suffers from poor performance for all the investigated network sizes. 
