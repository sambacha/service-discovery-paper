...
\para{Setup} We implemented a prototype of \sysname in PeerSim~\cite{p2p09-peersim}, a scalable P2P network simulator. To that end, we first implemented the Ethereum DHT by extending a vanilla Kademlia implementation~\cite{peersim_kademlia} with the required changes discussed in ~\Cref{sec:background:dht}. We then built \sysname on top of the Ethereum DHT implementation.
...
As similarly done in the IPFS system, we use N=20 peers (that are closest to the desired topic's hash) for storage of ads in both \altname and \altnameticket approaches.
