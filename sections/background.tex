...
To enhance the security of lookups, \er{detail what is the risk that is prevented}\mk{This is to prevent the queried malicious nodes to create on the fly and return to us nodes that are extremely close to the target key. If we don't expose the key, this risk is reduced.} Ethereum nodes provide a distance to a key (instead of the target itself) where distance is defined as the bucket number of the key where 
