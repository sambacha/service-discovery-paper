...
%\textbf{Lookups.}
%
The bucket partitioning scheme divides the keyspace from the point of view of $X$ into disjoint intervals, halving in length every time the bucket's associated prefix includes one more common bit with $X$'s ID.
...
\er{It seems that the Ethereum Kademlia does not support at all lookups but only random walks? We should probably clarify this to avoid confusion if we cannot present that as a non-secure baseline in the evaluation section.}
...
To enhance the security of lookups, \er{detail what is the risk that is prevented} Ethereum nodes provide a distance to a key (instead of the target itself) where distance is defined as the bucket number of the key where 
...
In \discv, Ethereum's current service discovery protocol, nodes perform \textit{random walks} over the DHT overlay structure and perform application-layer \emph{handshakes} with the encountered peers.
...
Peer discovery through random walks is reasonably resilient against eclipse attacks. Attackers cannot strategically place their Sybil identities in the keyspace to increase their chance of being discovered by victims, \ie, all locations in the keyspace have an equal chance of being discovered. % \er{consider replacing discovered by scanned or interrogated?}
...
More importantly, for applications with low popularity, the number of handshakes can be excessive, \ie on the order of thousands when 0.1\% of the nodes are running the target application. \er{in addition to the numbers in the intro, can we give more precise numbers here? One of the reviewer did not get this central point of \discv inefficiency.}
Because all the applications are initially unpopular, the upfront cost of building a sub-network can be large and finding peers can take a long time. \er{can we give more precise numbers based on simulations or experiments?}\mk{We can, it's just a bit challenging as it's size dependent. Would some examples be ok? If not, we can also refer the readers to our evaluation}
